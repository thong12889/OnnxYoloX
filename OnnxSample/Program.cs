...
namespace OnnxSample
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            /*var detector = new YoloDetector("model/wally640x640.onnx");
            using (var image = Cv2.ImRead("wally_img/1_1.jpg"))
            {
                //float ratio = 0.0f;
                //Point diff1 = new Point();
                //Point diff2 = new Point();
                //var letter_image = YoloDetector.CreateLetterbox(image, new Size(640, 640), new Scalar(114, 114, 114), out ratio, out diff1, out diff2);
            ...
            //-------------Inference folder image----------//

            string[] filepath = Directory.GetFiles("wally_img_640x640");
            var detector = new YoloDetector("model/wally640x640.onnx");

            foreach (var fp in filepath)
            {
                using (var image = Cv2.ImRead(fp))
